...
describe("the JavaScript language", function(){
  ...
  describe("considers functions as first class citizens", function(){
	...
	it("may return arrays that contains functions and so on", function(){
		function example(){
			return [function myFunc(x){return [0,10]}]   
		}
  ...
  describe("has multiple ways to define and create objects", function(){
	...
	it("may create objects also with the module pattern", function(){
		function createObject(initialScore, initialcolor){
			...
			return {
				incrementScoreIn: function(a){ score += a; },
				color: color,
				points: function(a){ return score }
			 };
		}
	...
	it("can have methods in the prototype", function(){
		function Obj(){
		  var name = 'bob';
		}
  ...
  describe("commons patterns with functions and behaviors", function(){
	...
	it("hoists variables the way you probably dont expect", function(){  //RAROO
		function generateFunctionFor(j){
			return function(){
				return j;
			};
		}
		function generate(){
		  var functions = [];
		  for (var i = 0; i < 5; i++){
			functions.push(generateFunctionFor(i));
		  }
...
//it("hoists variables the way you probably dont expect - solved with IIFE", function(){  //RAROO
  ...
  context("has ways to simulate classes", function(){
	...
	describe("event driven development", function(){
		...
		it("returning function inside function", function(){
			var x = 1;
		...
		it("returning elements of array", function(){
	...
	describe("Namespace examples", function(){

		it("could be this implementation", function(){
			var myNamespace = {};

			(function(ns){
				
				var count = 0;
				ns.currentCounter = function(){
					return count;
				};
				...
				ns = {} // this line doesn't erase the previous functions but the later

				ns.decrement = function(){
					count--;
				};
			...
			expect(myNamespace.currentCounter()).toBe(1);

			expect(myNamespace.decrement).toBe(undefined);

		});

		it("other implementation", function(){
			...
			expect(yourNamespace.foo()).toBe(1);
		});
